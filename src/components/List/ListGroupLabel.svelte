<script lang="ts" module>
	import type { Snippet } from 'svelte';
	import type { HTMLAttributes } from 'svelte/elements';
	import type { WithElementRef } from '../../types';

	export type ListGroupLabelProps = WithElementRef<
		{
			/** Caption content (small uppercase). */
			children?: Snippet;
			/** Render-delegation: receive the merged props and render your own element. */
			child?: Snippet<[{ props: Record<string, unknown> }]>;
		} & Omit<HTMLAttributes<HTMLDivElement>, 'children'>
	>;
</script>

<script lang="ts">
	import { createAttachmentKey } from 'svelte/attachments';
	import { mergeProps } from '../../utils/mergeProps';
	import { attachRef } from '../../utils/ref';
	import { useListGroupContext } from './context';

	let {
		children,
		child,
		ref = $bindable(null),
		class: className,
		style: userStyle,
		...rest
	}: ListGroupLabelProps = $props();

	const group = useListGroupContext();
	const fallbackId = $props.id();
	const labelId = group?.labelId ?? fallbackId;

	$effect(() => {
		group?.setLabelled();
	});

	const attrs = $derived({
		class: 'list-group__label',
		id: labelId,
		'data-testid': 'list-group-label',
		style: userStyle
	});
	const merged = $derived(
		mergeProps(rest, attrs, {
			class: className,
			[createAttachmentKey()]: attachRef<HTMLDivElement>((n) => (ref = n))
		})
	);
</script>

{#if child}
	{@render child({ props: merged })}
{:else}
	<div {...merged}>{@render children?.()}</div>
{/if}

<style>
	.list-group__label {
		padding: var(--space-3) var(--space-6) var(--space-2);
		font-size: var(--fs-xs);
		text-transform: uppercase;
		letter-spacing: 0.08em;
		font-weight: 600;
		opacity: 0.5;
		-webkit-user-select: none;
		user-select: none;
	}
</style>
